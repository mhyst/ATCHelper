package ivao;
import atchelper.LogOutputStream;
import java.io.*;
import java.net.*;import java.util.Hashtable;
...
public class SocksProxy implements Runnable {
    ...
    private IvaoServer is;

    public SocksProxy(String[] args, IvaoServer is) throws IOException, UnknownHostException
    {		
        this.is = is;
        try {
            SERVER = args[0];
            ...
            try {
                ...
                while (true) {
                    try {
                        ...
                        data = destin.read();
                        byte[] buff = new byte[destin.available()+1];
                        int i = 0;
                        while (destin.available() > 0) {
                            buff[i] = (byte) data;
                            srcout.write(data);
                        ...
                        if (data == -1) {
                            is.incoming(new String(buff, "UTF8"));
                            break;
                        } else {
                            srcout.write(data);
                            buff[i] = (byte) data;
                    ...
                    try {
                        data = srcin.read();
                        byte[] buff = new byte[srcin.available()+1];
                        int i = 0;
                        while (srcin.available() > 0) {
                            buff[i] = (byte) data;
                            srcout.write(data);
                        ...
                        if (data == -1) {
                            is.outgoing(new String(buff, "UTF8"));
                            break;
                        } else {
                            destout.write(data);
                            buff[i] = (byte) data;
